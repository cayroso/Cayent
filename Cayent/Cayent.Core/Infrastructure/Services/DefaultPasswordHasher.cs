...
namespace Cayent.Core.Infrastructure.Services
{
    ...
    {
        string IPasswordHasher.GetSaltedPassword(string password, string salt)
        {
        ...
        string IPasswordHasher.Salt()
        {
            return GenerateSalt(SaltSize);
        }

        string IPasswordHasher.Hash(string password, string salt)
        {
            var saltedPassword = (this as IPasswordHasher).GetSaltedPassword(password, salt);
        ...
        bool IPasswordHasher.VerifyHashedPassword(string hashedPassword, string password)
        {
        ...
        internal byte[] GenerateSaltInternal(int byteLength = SaltSize)
        {
        ...
        [SuppressMessage("Microsoft.Naming", "CA1720:IdentifiersShouldNotContainTypeNames", MessageId = "byte", Justification = "It really is a byte length")]
        public string GenerateSalt(int byteLength = SaltSize)
        {
